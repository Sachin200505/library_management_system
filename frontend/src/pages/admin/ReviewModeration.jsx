                    ...
                    {reviews.map((review) => (
                        <div key={review.id} className="p-4 space-y-4">
                            <div className="flex justify-between items-start">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <h3 className="font-bold text-slate-900 leading-tight">
                                            {review.user_username}
                                            {review.user === user?.id && <span className="ml-2 text-[10px] bg-blue-50 text-blue-600 px-1.5 py-0.5 rounded-full font-bold border border-blue-100 shadow-sm">YOU</span>}
                                        </h3>
                                        <div className="flex items-center gap-2 mt-1">
                                            <div className="flex items-center text-amber-500">
                                                <Star className="w-3 h-3 fill-current" />
                                                <span className="ml-1 text-[11px] font-black">{review.rating}</span>
                                            </div>
                                            <span className={`px-2 py-0.5 rounded-full text-[9px] font-black border uppercase tracking-widest ${review.status === 'APPROVED' ? 'bg-green-50 text-green-700 border-green-100' :
                                                review.status === 'HIDDEN' ? 'bg-slate-100 text-slate-600 border-slate-100' :
                                ...
                                <div className="text-[10px] text-slate-400 font-bold bg-slate-50 px-2 py-1 rounded border border-slate-100 uppercase">
                                    Ref: #{review.id}
                                </div>
                            </div>
                            ...
                            <div className="space-y-3">
                                <div className="flex items-start gap-2 bg-slate-50 p-2.5 rounded-xl border border-slate-100">
                                    ...
                                    <div className="space-y-1">
                                        <p className="text-[10px] text-slate-400 font-black uppercase tracking-wider">Book Title</p>
                                        <p className="text-sm font-bold text-slate-800 leading-tight">{review.book_title || review.book?.title}</p>
                                    </div>
                                </div>
                                <div className="bg-white p-3 rounded-xl border border-slate-200/60 shadow-inner">
                                    <p className="text-[10px] text-slate-400 font-black uppercase tracking-wider mb-1.5">User Experience</p>
                                    <p className="text-xs text-slate-600 leading-relaxed italic">"{review.text || 'No comments provided'}"</p>
                                </div>
                            </div>
                            ...
                            {isAdminOrOwner && (
                                <div className="flex items-center gap-2 pt-2">
                                    {review.status !== 'APPROVED' && (
                                        <button
                                            onClick={() => handleStatusChange(review.id, 'APPROVED')}
                                            className="flex-1 py-2.5 bg-emerald-600 text-white font-bold rounded-xl shadow-lg shadow-emerald-500/20 flex items-center justify-center gap-2 active:scale-95 transition-all"
                                        >
                                    ...
                                    {review.status !== 'HIDDEN' && (
                                        <button
                                            onClick={() => handleStatusChange(review.id, 'HIDDEN')}
                                            className="flex-1 py-2.5 bg-slate-100 text-slate-600 font-bold rounded-xl border border-slate-200 flex items-center justify-center gap-2 active:scale-95 transition-all"
                                        >
